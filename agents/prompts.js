...
function imageSys(cfg, sceneCount) {
  ...
  return [
    ...
    `2. Каждый prompt — на АНГЛИЙСКОМ, начинается со СТРОГОГО блока идентификации: "Use the attached character reference image as the STRICT and ONLY source of truth. Reproduce the EXACT same character from the reference image — identical face, eyes, body shape, colours, proportions, plant species, leaves, fruits/tubers and art style. This is a ${cfg.crop_en.toUpperCase()} plant character (${cfg.char_name}). Do NOT redesign the character and do NOT change the plant species into another vegetable."`,
    `3. Сохраняй персонажа: same face, eyes, smile, body, arms, base/roots, foliage, ${cfg.crop_en}-specific details. Явно укажи вид: ${cfg.crop_en} (НЕ томат, НЕ клубника, НЕ огурец, НЕ картофель — кроме текущего).`,
