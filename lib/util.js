 ...
 * If the local file exists, use it. Otherwise download from GitHub raw
 * (so a fresh clone / server without the binary still works).
 */
async function ensureRefLocal(cfg, repoRoot) {
  ...
  const base = process.env.REPO_RAW_BASE ||
    'https://raw.githubusercontent.com/trumpa703-design/cartoon-plants/main/';
  const remote = base.replace(/\/$/, '') + '/' + cfg.reference.replace(/^\/+/, '');
  const dest = local;
  console.log('[ref] local missing, fetching from GitHub: ' + remote);
